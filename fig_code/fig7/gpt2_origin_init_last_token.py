...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # 找到每个序列中token_id=25673的位置
            target_token_positions = (input_ids == 16289).nonzero(as_tuple=True)
            batch_indices, seq_positions = target_token_positions
            
            # 确保每个序列都有目标token
            if len(batch_indices) != input_ids.size(0):
                raise ValueError("每个序列中都应该有token_id=16289")
            
            # 创建mask，只在目标token之后的位置为True
            mask = torch.zeros_like(input_ids, dtype=torch.bool)
            for i, (b, p) in enumerate(zip(batch_indices, seq_positions)):
                mask[b, p+1:] = True
            
            # 应用mask到logits和labels
            shift_logits = lm_logits[:, :-1][mask[:, :-1]]
            shift_labels = labels[:, 1:][mask[:, :-1]]
